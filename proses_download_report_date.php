...
$periodeStart = $_POST['periode_start'];
$periodeEnd = $_POST['periode_end'];
$updatedProjectCode = $_POST['kode_project'];

// var_dump($periodeStart);
// var_dump($periodeEnd);
...
if ($selectedProjectCode == 'ALL') {
    ...
    $filename = 'report_ALL_' . $periodeStart . '_' . $periodeEnd .'.xlsx';
    $selectedProjectCode = substr_replace($stringListKodeProject ,"",-1);
}
else{
    $filename = 'report_' . $selectedProjectCode . '_' . $periodeStart . '_' . $periodeEnd .'.xlsx';
...
$projectNameQuery = "SELECT nama_project FROM master_project WHERE kode_project IN ($selectedProjectCode) AND `start_date` BETWEEN '$periodeStart' AND '$periodeEnd'";
$projectNameResult = mysqli_query($konek, $projectNameQuery);


if(mysqli_num_rows($projectNameResult) > 0) {
    if($updatedProjectCode == 'ALL') {
        $nama_project = 'ALL';
    }
    else{
...
}elseif(mysqli_num_rows($projectNameResult) == 0 && $updatedProjectCode == 'ALL') {
    $nama_project = 'ALL';
}
else{
    $nama_project ='Project Name Not Found';
}
// var_dump($updatedProjectCode);
// var_dump($nama_project);
// exit();
...
AND log_trs.start_date BETWEEN '$periodeStart' AND '$periodeEnd'";
...
function countDays($selectedProjectCode, $konek, $kode_proses, $periodeStart, $periodeEnd){
    ...
    $cellH4->setValue('Kode Project : ' . $updatedProjectCode); // Update with the selected project code
    ...
    $cellH5 = $worksheet->getCell('A6');
    $cellH5->setValue('Periode : ' . $periodeStart . ' s.d ' . $periodeEnd); // Use the project name fetched from the database
    $cellH5->getStyle()->getFont()->setBold(false);
    $cellH5->getStyle()->getFont()->setSize(12); 
...
if (mysqli_num_rows($projectResult) > 0) {
    ...
    $worksheet->setCellValue('A8', 'Start Project');
    $worksheet->setCellValue('B8', 'Kode Project');
    $worksheet->setCellValue('C8', 'Nama Project');
    $worksheet->setCellValue('D8', 'Kode Proses');
    $worksheet->setCellValue('E8', 'Proses');
    $worksheet->setCellValue('F8', 'Scope');
    $worksheet->setCellValue('G8', 'NIK');
    $worksheet->setCellValue('H8', 'Nama User');
    $worksheet->setCellValue('I8', 'Role');
    $worksheet->setCellValue('J8', 'Tgl Transaksi');
    $worksheet->setCellValue('K8', 'Start Date');
    $worksheet->setCellValue('L8', 'End Date');
    $worksheet->setCellValue('M8', 'SLA Per User');
    $worksheet->setCellValue('N8', 'STD SLA Per Proses');
    $worksheet->setCellValue('O8', 'SLA Per Proses');



    // Fetch data and place it in cells
    $row = 9; // Start from the second row
    ...
    while ($data = mysqli_fetch_assoc($projectResult)) {
        ...
        if($data['log_kode_proses'] == 'P0001'){
            ...
            $totalSLA = countDays($selectedProjectCode, $konek, $data['log_kode_proses'], $periodeStart, $periodeEnd);
