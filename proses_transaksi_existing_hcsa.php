...
$insert_query = "INSERT INTO log_trs (kode_trs, kode_project, nik, kode_role, start_date, end_date, kode_proses, next_proses, next_user, sla_hcsa) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$sla_hcsa = hitungSLA($start_date, $end_date, $libur, $weekend);

// Bind the parameters
$stmt->bind_param("ssssssssss", $kode_trs, $kode_project, $nik, $kode_role, $start_date, $end_date, $kode_proses, $next_proses, $next_user, $sla_hcsa);

if ($stmt->execute()) {

    // Get the current value of sla_hcsa from master_project
    $select_current_sla_query = "SELECT sla_hcsa FROM master_project WHERE kode_project = ?";
    ...
    // Calculate the new value of sla_hcsa by adding the current value and the new sla_hcsa
    $new_sla_hcsa = $current_sla + $sla_hcsa;
    switch($next_proses){
        case 'P0001':
        $kode_proses = 'P0001';
        break;
    ...
    $stmt_update = $konek->prepare("UPDATE master_project SET sla_hcsa = ?, flag = ?, kode_proses = ? WHERE kode_project = ?");
    
    // Bind the parameters for updating
    $stmt_update->bind_param("ssss", $new_sla_hcsa, $flag, $kode_proses, $kode_project);
