...
// Get values for $pic_user, $pic_hcsa, $pic_itGm, $pic_itDev, $pic_itQa, and $pic_itRoll from the master_project table
$select_project_query = "SELECT pic_user, pic_hcsa, pic_itGm, pic_itDev, pic_itQa, pic_itRoll FROM master_project WHERE kode_project = ?";
...
$stmt_select_project->bind_result($pic_user, $pic_hcsa, $pic_itGm, $pic_itDev, $pic_itQa, $pic_itRoll);
...
$insert_query = "INSERT INTO log_trs (kode_trs, kode_project, nik, kode_role, start_date, end_date, kode_proses, next_proses, next_user, sla_itScrum) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$sla_itScrum = hitungSLA($start_date, $end_date, $libur, $weekend);

// Bind the parameters
$stmt->bind_param("ssssssssss", $kode_trs, $kode_project, $nik, $kode_role, $start_date, $end_date, $kode_proses, $next_proses, $next_user, $sla_itScrum);

if ($stmt->execute()) {

    // Get the current value of sla_itScrum from master_project
    $select_current_sla_query = "SELECT sla_itScrum FROM master_project WHERE kode_project = ?";
    ...
    // Calculate the new value of sla_itScrum by adding the current value and the new sla_itScrum
    $new_sla_itScrum = $current_sla + $sla_itScrum;
    switch($next_proses){
        case 'P0001':
        $kode_proses = 'P0001';
        break;
    ...
    $stmt_update = $konek->prepare("UPDATE master_project SET sla_itScrum = ?, flag = ?, kode_proses = ? WHERE kode_project = ?");
    
    // Bind the parameters for updating
    $stmt_update->bind_param("ssss", $new_sla_itScrum, $flag, $kode_proses, $kode_project);
