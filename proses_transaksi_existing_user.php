...
// Get values for $pic_hcsa, $pic_user, $pic_itScrum, $pic_itGm, $pic_itDev, $pic_itQa, and $pic_itRoll from the master_project table
$select_project_query = "SELECT pic_hcsa, pic_user, pic_itScrum, pic_itGm, pic_itDev, pic_itQa, pic_itRoll FROM master_project WHERE kode_project = ?";
...
$stmt_select_project->bind_result($pic_hcsa, $pic_user, $pic_itScrum, $pic_itGm, $pic_itDev, $pic_itQa, $pic_itRoll);
$stmt_select_project->fetch();
$stmt_select_project->close();

// Inisialisasi $flag berdasarkan nilai $next_user
if ($next_user == $pic_hcsa) {
    $flag = 1;
...
$insert_query = "INSERT INTO log_trs (kode_trs, kode_project, nik, kode_role, start_date, end_date, kode_proses, next_proses, next_user, sla_user) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$sla_user = hitungSLA($start_date, $end_date, $libur, $weekend);

// Bind the parameters
$stmt->bind_param("ssssssssss", $kode_trs, $kode_project, $nik, $kode_role, $start_date, $end_date, $kode_proses, $next_proses, $next_user, $sla_user);

if ($stmt->execute()) {

    // Get the current value of sla_user from master_project
    $select_current_sla_query = "SELECT sla_user FROM master_project WHERE kode_project = ?";
    ...
    // Calculate the new value of sla_user by adding the current value and the new sla_user
    $new_sla_user = $current_sla + $sla_user;
    switch($next_proses){
        case 'P0001':
        $kode_proses = 'P0001';
        break;
    ...
    // var_dump($kode_proses);
    // exit();
    // Create a prepared statement for updating the user
    $stmt_update = $konek->prepare("UPDATE master_project SET sla_user = ?, flag = ?, kode_proses = ? WHERE kode_project = ?");

    // Bind the parameters for updating
    $stmt_update->bind_param("ssss", $new_sla_user, $flag, $kode_proses, $kode_project);
